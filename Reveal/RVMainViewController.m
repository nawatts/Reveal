...
- (void)dealloc
{
  [_compass_display release];
  [_map_display release];
  [super dealloc];
}
...
- (void)layoutSubviews
{
  CGFloat y_offset = [[UIApplication sharedApplication] isStatusBarHidden] ? 20 : 0;
  
  if ( UIInterfaceOrientationIsPortrait(self.interfaceOrientation) ) {
    self.map_display.frame = CGRectMake(8, 362+y_offset, 90, 90);
    self.compass_display.frame = CGRectMake(245, 422+y_offset, 67, 30);
  }
  else {
    self.map_display.frame = CGRectMake(8, 202+y_offset, 90, 90);
    self.compass_display.frame = CGRectMake(405, 262+y_offset, 67, 30);
  }
}
...
- (void)viewDidLoad
{
  ...
  [self.view addSubview:self.compass_display];
  ...
  [self.view addSubview:self.map_display];
  
  // Position subviews correctly for current orientation
  [self layoutSubviews];
}
